...
class StepsRepository(val jdbcTemplate: JdbcTemplate) {
    ...
    fun save(stepDto: StepDto): StepDao {
        ...
        """.trimIndent()
        return jdbcTemplate.queryForObject(sql, StepDaoRowMapper(), stepDto.ordering, stepDto.action, stepDto.expectedResult, stepDto.actualResult, stepDto.status, stepDto.testCaseId)!!
    }
    ...
    fun update(id: Int, stepDto: StepDto): StepDao? {
        val sql = """
            UPDATE steps
            SET ordering = ?, action = ?, expected_result = ?, actual_result = ?, status = ?, test_case_id = ?
            WHERE id = ?
            RETURNING id, ordering, action, expected_result, actual_result, status, test_case_id
        """.trimIndent()
        return jdbcTemplate.queryForObject(sql, StepDaoRowMapper(), stepDto.ordering, stepDto.action, stepDto.expectedResult, stepDto.actualResult, stepDto.status, stepDto.testCaseId, id)
    }
    ...
    internal class StepDaoRowMapper : RowMapper<StepDao> {
        ...
        override fun mapRow(rs: ResultSet, rowNum: Int): StepDao {
            ...
            stepDao.status = rs.getBoolean("status")
